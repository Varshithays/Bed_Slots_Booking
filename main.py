...
local_server=True
app=Flask(__name__)
app.secret_key="sjbitdbmsproject"
...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:@localhost/bedslots'
db=SQLAlchemy(app)
...
class Bookingpatients(db.Model):
    id=db.Column(db.Integer,primary_key=True)
    srfid=db.Column(db.String(20),unique=True)
...
def admin():
    ...
    if request.method=="POST":
        ...
        password=request.form.get('password')
        if(username=="bedslot" and password=="vs123"):
...
def hospitalUser():
    ...
    if('user' in session and session['user']=="bedslot"):
        ...
        if request.method=="POST":
           ...
           # mail.send_message('Online Bed Slot Booking Platform ',sender=params['gmail-user'],recipients=[email],body=f"Welcome thanks for choosing us\nYour Login Credentials Are:\n Email Address: {email}\nPassword: {password}\n\nHospital Code {hcode}\n\n Do not share your password\n\n\nThank You..." )
...
def pdetails():
    ...
    print(code)
    data=Bookingpatients.query.filter_by(srfid=code).first()
...
def slotbooking():
    ...
    if request.method=="POST":
        ...
        checkpatient=Bookingpatients.query.filter_by(srfid=srfid).first()
        
        if int(spo2) < 75:
            flash("Go to hospital Immediately!Your oxygen level is too low","danger")
        if checkpatient:
        ...
        if bedtype=="NormalBed":       
            for d in dbb:
                ...
                if seat==0:
                    flash("No Beds are Available","danger")
                    return render_template("booking.html",query=query,query1=query1)
                else:
                    ar.normalbed=seat-1
                    db.session.commit()
        ...
        if check!=None:
            if(seat>0 and check):
                res=Bookingpatients(srfid=srfid,bedtype=bedtype,hcode=hcode,spo2=spo2,pname=pname,pphone=pphone,paddress=paddress)
                db.session.add(res)
                db.session.commit()
